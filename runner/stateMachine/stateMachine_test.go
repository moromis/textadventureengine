...
func TestParseMovement(t *testing.T) {
	...
	tests := []struct {
		...
		{"go", southArray, testObjects.Here.ValidMoves[SOUTH]}, // TODO: in the future this will actually respond with the movement response from the room?
		{"walk", emptyArray, constants.WHERE_TO_GO("walk")},
		{"walk", northArray, testObjects.There.ValidMoves[NORTH]}, // TODO: in the future this will actually respond with the movement response from the room?
	}
